...
| `size` | string | No | `'normal'` | Size variant: `'normal'`, `'compact'` |
...
```lua copy
-- Normal size (default) - full UI with instructions
...
```lua copy
-- Compact size - minimal UI (just circle and key)
local success = exports.lation_ui:skillCheck('Lockpick', {'easy', 'easy', 'easy', 'easy'}, {'W', 'A', 'S', 'D'}, 'compact')
