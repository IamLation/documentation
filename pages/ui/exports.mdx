---
title: Advanced UI - Exports
description: Available exports for Advanced UI
---
...
|--------|------|----------|---------|-------------|
| `title` | string | No* | `nil` | The dialog title |
| `message` | string | No* | `nil` | The main message text |
...
| `cancelText` | string | No | `'Cancel'` | Cancel button text (set to `nil` to hide) |
| `type` | string | No | `'default'` | Dialog type: `'default'`, `'success'`, `'destructive'` |
| `maxWidth` | string | No | `'md'` | Max width: `'xs'`, `'sm'`, `'md'`, `'lg'` |
...
| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `type` | string | Yes | `nil` | Field type: `'text'`, `'number'`, `'toggle'`, `'select'`, `'multiselect'`, `'slider'`, `'textarea'` |
...
| `options` | table | No | `{}` | Options for select/multiselect |
...
Progress bars provide visual feedback for long-running tasks with customizable options.
